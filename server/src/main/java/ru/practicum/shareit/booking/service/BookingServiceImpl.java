...
public class BookingServiceImpl {
    ...
    public List<BookingDto> getAllBooking(long userId, BookingRequestType status, int from, int size) {
        ...
        switch (status) {
            case ALL -> bookingPage = bookingRepository.findByBookerIdOrderByStartDesc(userId, pageable);
            case CURRENT -> bookingPage = bookingRepository.findByBookerIdAndCurrent(userId, now, pageable);
            case PAST ->
                    bookingPage = bookingRepository.findByBookerIdAndEndBeforeOrderByStartDesc(userId, now, pageable);
            case FUTURE ->
                    bookingPage = bookingRepository.findByBookerIdAndStartAfterOrderByStartDesc(userId, now, pageable);
            case WAITING, REJECTED -> bookingPage = bookingRepository.findByBookerIdAndStatusIsOrderByStartDesc(userId,
                    BookingStatus.valueOf(status.name()), pageable);
            default -> throw new NotFoundException("Неверные параметры запроса");
        }

        return bookingPage.getContent().stream().map(BookingMapper::toBookingDto).toList();
    }
